...
def test_between_rule_gt_equals(widget, get_renderer):
    widget.rules = []
    widget.rules.append(rules.BetweenRule('vis', 'dummy_pv', '0', '5', gt_equals=True))
    renderer = get_renderer(widget)
    renderer.assemble()
    rule_element = renderer.get_node().find('./rules/rule')
    assert rule_element.find('./pv').text == 'dummy_pv'
...
def test_between_rule_lt_equals(widget, get_renderer):
    widget.rules = []
    widget.rules.append(rules.BetweenRule('vis', 'dummy_pv', '0', '5', lt_equals=True))
    renderer = get_renderer(widget)
    renderer.assemble()
    rule_element = renderer.get_node().find('./rules/rule')
    assert rule_element.find('./pv').text == 'dummy_pv'
...
def test_between_rule_gt_equals_and_lt_equals(widget, get_renderer):
    widget.rules = []
    widget.rules.append(rules.BetweenRule('vis', 'dummy_pv', '0', '5', gt_equals=True, lt_equals=True))
    renderer = get_renderer(widget)
    renderer.assemble()
    rule_element = renderer.get_node().find('./rules/rule')
    assert rule_element.find('./pv').text == 'dummy_pv'
